...
public class DescriptorMatcher extends BaseMatcher<DescriptorMatch> {
  ...
  protected DescriptorMatch tupleToMatch(final Tuple t) {
    try {
    	return DescriptorMatch.newMatch((hu.bme.mdsd.ztz.model.drone.TaskRequirement) t.get(POSITION_SOURCEREQUIREMENT), (hu.bme.mdsd.ztz.model.drone.TaskDescriptor) t.get(POSITION_TARGETDESCRIPTOR));
  ...
  protected DescriptorMatch arrayToMatch(final Object[] match) {
    try {
    	return DescriptorMatch.newMatch((hu.bme.mdsd.ztz.model.drone.TaskRequirement) match[POSITION_SOURCEREQUIREMENT], (hu.bme.mdsd.ztz.model.drone.TaskDescriptor) match[POSITION_TARGETDESCRIPTOR]);
    } catch(ClassCastException e) {
  ...
  protected DescriptorMatch arrayToMatchMutable(final Object[] match) {
    try {
    	return DescriptorMatch.newMutableMatch((hu.bme.mdsd.ztz.model.drone.TaskRequirement) match[POSITION_SOURCEREQUIREMENT], (hu.bme.mdsd.ztz.model.drone.TaskDescriptor) match[POSITION_TARGETDESCRIPTOR]);
    } catch(ClassCastException e) {
    	LOGGER.error("Element(s) in array not properly typed!",e);
    	return null;
    }
